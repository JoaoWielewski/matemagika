<?php 
    ...
    function formatImages($unformattedText) {
        ...
        return str_replace('(_imagem)', '" alt=""><br>', $semiFormattedText);
    ...
    if (!empty($_SESSION['content'])) {
        ...
        $examQuery = $pdo->query("SELECT E.IDEXAM
                                  ...
                                  WHERE C.IDCONTENT = '$contentID' ");
        $examArray = $examQuery->fetch(PDO::FETCH_ASSOC);
        $examID = $examArray['IDEXAM'];

        $examQuestionsQuery = $pdo->query("SELECT ID_QUESTION 
                                           FROM EXAMS_QUESTIONS
                                           WHERE ID_EXAM = '$examID' ");
        $examQuestionsArray = $examQuestionsQuery->fetchAll(PDO::FETCH_ASSOC);

        foreach($examQuestionsArray as $question){
...
<body>
    ...
    <div class="lesson-page flex">
        ...
        <div class="exam-div">
            <div class="exam-name">Prova de <?php echo $contentName ?></div>

                <?php 
                
                if($examQuestionsArray){
                    $counter = 1;

                    echo '<div class="exam-question-separator"></div>
                    ...
                    foreach($examQuestionsArray as $question){
                        ...
                        echo '<div class="question">
                              <div class="lesson-question">
                               <div class="exam-question-number">' . $counter . '</div>';
                            ...
                            echo '<div class="alternatives-div">
                                <div class="question-alternative-div">
                                    <input type="radio" name="alternative-' . $counter . '" value="1" id="alternative-' . $counter . '-1" class="exam-question-alternative">
                                    <label for="alternative-' . $counter . '-1" class="alternative-label">' . formatImages($alternative1) . '</label><br>
                                </div>
                                <div class="question-alternative-div">
                                    <input type="radio" name="alternative-' . $counter . '" value="2" id="alternative-' . $counter . '-2" class="exam-question-alternative">
                                    <label for="alternative-' . $counter . '-2" class="alternative-label">' . formatImages($alternative2) . '</label><br>
                                </div>
                                <div class="question-alternative-div">
                                    <input type="radio" name="alternative-' . $counter . '" value="3" id="alternative-' . $counter . '-3" class="exam-question-alternative">
                                    <label for="alternative-' . $counter . '-3" class="alternative-label">' . formatImages($alternative3) . '</label><br>
                                </div>
                                <div class="question-alternative-div">
                                    <input type="radio" name="alternative-' . $counter . '" value="4" id="alternative-' . $counter . '-4" class="exam-question-alternative">
                                    <label for="alternative-' . $counter . '-4" class="alternative-label">' . formatImages($alternative4) . '</label><br>
                                </div>
                                <div class="question-alternative-div">
                                    <input type="radio" name="alternative-' . $counter . '" value="5" id="alternative-' . $counter . '-5" class="exam-question-alternative">
                    ...
                    echo '</div>
                          <div class="exam-question-separator"></div>';
                      ...
                      if(isset($_GET['consult'])){
                        ...
                        echo '<div class="exam-name-question">Resultados</div>
                        ...
                        foreach($examQuestionsArray as $question){
                            ...
                            $questionResult = ($result[$resultCounter] != 0) ? $result[$resultCounter] . '° Alternativa' : 'Nada marcado';

                            if($questionCorrectResult['CORRECT'] == $result[$resultCounter]) {
                                echo '<div class="exam-question-result">
                                        <div class="exam-question-number">' . $questionNumber . '</div>
                        ...
                        echo '</div>
                              <div class="exam-result-separator"></div>';      
